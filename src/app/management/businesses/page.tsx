...
import { Camera, Search, Undo2, X } from "lucide-react";
...
export default function ManageBusinesses() {
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      params.set("filter", "name:contains:" + encodeURIComponent(searchQuery.trim()));
    }
    const qs = params.toString();
    fetch("/api/management/businesses" + (qs ? "?" + qs : ""))
      .then(async (r) => {
        if (!r.ok) throw new Error(await readProblemMessage(r, "Failed to load businesses"));
        return r.json();
      })
      .then((data: Business[]) => { if (isMounted) { setBusinesses(data); setIsLoading(false); } })
      .catch((err) => { if (isMounted) { setError(err instanceof Error ? err.message : "Failed to load businesses"); setIsLoading(false); } });
