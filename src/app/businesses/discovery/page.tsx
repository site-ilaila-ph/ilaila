...
export default function BusinessDiscoveryPage() {
  ...
  return (
    <main className="min-h-screen bg-brand-bg text-foreground">
      ...
      <section className="border-b border-brand-border bg-brand-bg-accent px-6 py-16 sm:py-24"><div className="mx-auto max-w-7xl"><p className="mb-5 text-sm font-semibold uppercase tracking-[0.18em] text-brand-accent">San Pedro, Laguna</p><div className="max-w-3xl"><h1 className="font-heading text-5xl font-bold leading-[0.98] tracking-tight sm:text-7xl">Tuklasin ang iyong susunod na paboritong lugar.</h1><p className="mt-6 max-w-xl text-lg leading-relaxed text-muted-foreground">Mga independiyenteng restawran, kapitbahay na kapehan, at mga lugar na sulit puntahan.</p></div><div className="mt-10 flex max-w-2xl items-center gap-3 rounded-2xl border border-brand-border bg-card p-2 shadow-sm"><Search className="ml-3 size-5 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Maghanap ng lugar, pagkain, o kapitbahayan" className="min-w-0 flex-1 bg-transparent px-1 py-3 text-sm outline-none text-foreground placeholder:text-muted-foreground" /><Button size="lg" className="hidden sm:inline-flex">Hanapin</Button></div></div></section>
